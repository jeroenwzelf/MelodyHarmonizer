const Genetic = (function() {

    // facilitates communication between web workers
    const Serialization = {
    ...
    function Genetic() {
        ...
        this.start = function() {
            ...
            for (i=0;i<this.configuration.iterations;++i) {
                ...
                let mean = pop.reduce(function (a, b) { return a + b.fitness; }, 0)/pop.length;
                ...
                let r = this.generation ? this.generation(pop.slice(0, this.configuration["maxResults"]), i, stats) : true;
                let isFinished = (typeof r != "undefined" && !r) || (i == this.configuration.iterations-1);
                ...
                while (newPop.length < self.configuration.size) {
                    if (
                        ...
                        let children = this.crossover(Clone(parents[0]), Clone(parents[1])).map(mutateOrNot);
    ...
    Genetic.prototype.evolve = function(config, userData) {
        ...
        // determine if we can use web workers
